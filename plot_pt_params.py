...
def load_pt_results(n_spins_list, m_replicas_list, results_dir, proposals_list, m_cg_list):
    """
    ...
    for i, n_spins in enumerate(n_spins_list):
        for j, m_replicas in enumerate(m_replicas_list):
            for k, proposals in enumerate(proposals_list):
                for l, m_cg in enumerate(m_cg_list):
                    ...
                    if replica_tag == "l"*len(proposals):
                        ...
                        res_path = results_dir / f"{str(n_spins).zfill(3)}_{m_replicas}_{replica_tag}_{m_cg}.pkl"
...
def plot_effort_vs_nhops(n_spins_list, m_replicas_list, m_quantum_replicas_list, results, output_dir, label, colormap, m_cg_list):
    """
    ...
    x_mins = np.zeros((len(spins_to_plot), len(m_replicas_list), len(m_quantum_replicas_list), len(m_cg_list), 3))  # Store x_min and dx_min for each subplot and results_list
    ...
    for j, n_spins_to_plot in enumerate(spins_to_plot):
        for k, m_replica in enumerate(m_replicas_list):
            for l, m_quantum_replica in enumerate(m_quantum_replicas_list):
                for i, m_cg in enumerate(m_cg_list):
                    ...
                    x_mins[j,k,l,i:] = [x_min, dx_min, y_min]
                    count+=1
    ...
    for i, m_replica in enumerate(m_replicas_list):
        for j, m_quantum_replica in enumerate(m_quantum_replicas_list):
            for k, m_cg in enumerate(m_cg_list):
                if len(m_quantum_replicas_list) >2 and len(m_cg) < 2:
                    colour = cmap(i * len(m_quantum_replicas_list) + j)
                elif len(m_quantum_replicas_list) <= 2 and len(m_cg) > 1:
                    colour = cmap(i * len(m_cg_list) + k)
                
                ax2.errorbar(spins_to_plot, x_mins[:, i, j, k, 0], yerr=x_mins[:, i, j,k, 1],
                            label=f'$M_q={m_quantum_replica}, M={m_replica}$, m_cg={m_cg_list[k]}', marker=markerslist[j], markersize=markersizelist[j], linewidth = 0.6, color = colour)
    
    ax2.set_xlabel('Number of Spins')
    ax2.set_ylabel('Optimal number of steps')
    ax2.set_title(f'Scaling of optimal number of steps for (Qe)PT')
    ax2.legend()
    # ax2.set_yscale('log')
    ...
    for i, m_replica in enumerate(m_replicas_list):
        for j, m_quantum_replica in enumerate(m_quantum_replicas_list):
            for k, m_cg in enumerate(m_cg_list):
                colour = cmap(i * len(m_quantum_replicas_list) + j)
                ax3.plot(spins_to_plot, x_mins[:, i, j,k, 2],
                            label=f'$M_q$={m_quantum_replica} m_cg = {m_cg_list[k]}', marker=markerslist[j],  linewidth = 0.6, color=colour, markersize=markersizelist[j])
    ax3.set_xlabel('Number of spins')
    ax3.set_ylabel('Optimal effort')
    ax3.set_title(f'Optimal effort vs number of spins')
    ax3.legend()
    ax3.set_yscale('log')
    ...
    plt.close(fig2)
    fig3.tight_layout()
    fig3.savefig(output_dir / f'PT_optimal_effort_vs_n_spins.png')
    plt.close(fig3)
...
def plot_hops_vs_probability(n_spins_list, m_replicas_list, m_quantum_replicas_list, results, output_dir, color, m_cg_list = []):
    """
...
if __name__ == "__main__":
    ...
    n_spins_list = n_spins = np.arange(4,25)#[4,5,6,7,8,9,10,12,14,15,16,18,20]
    ...
    results = load_pt_results(n_spins_list, m_replicas_list, results_dir, proposals_list=proposals_list, m_cg_list = m_cg_list)
    plot_effort_vs_nhops(n_spins_list, m_replicas_list, m_quantum_replicas_list , results, results_path, "", cmap, m_cg_list = m_cg_list)
    plot_hops_vs_probability(n_spins_list, m_replicas_list, m_quantum_replicas_list, results, results_path, "lightgreen", m_cg_list = m_cg_list)
